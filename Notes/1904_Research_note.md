...
#### 2019/04/25
##### 2PL受検者削減シミュレーションの続き
　設定を見直して，再実行。結果はRmarkdownでHTMLファイルにまとめた。

##### polytomous observed score distribution
　昨晩作ったアルゴリズムだと，min scoreの部分が正確に反映されていない。0から始まっても，1から始まっても良いのだが，min scoreを考えてもう一度プログラミングし直す必要がある。

#### 2019/04/26
##### Polytomous Observed Score Distribution
　多値復元得点分布関数を完成させた。まだ関数化のためのマニュアル関連を書いていないことと，GRM以外の多値モデルに対応させていないことをのぞいて，概ねOKだろう。(追記：のちにGPCMには対応させた。NRはまだ。)

##### mirtパッケージ
　GPCMの実行には"gpcm"と"gpcmIRT"の二種類がある。パラメタの分解の仕方が異なるため，両者の関係を理解しなくては，変換は難しそう。coef関数でIRTparsオプションをTにしても，変換結果は一致しなかった。どうやらmirtパッケージ流のパラメトリゼーションは，古典的な（Muraki流の)それとは大きく異なっているようである。

##### GRM未観測カテゴリ代入シミュレーション
　等間隔に代入するシミュレーションはOKなので，次はカテゴリ観測度数を基に代入項目パラメタを推定するプログラムを作る。
