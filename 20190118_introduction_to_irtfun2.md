...
##### 追記 20190410
`mirt`パッケージで多母集団モデルの分析ができました。ですが，母集団分布の推定が正確ではないので，使わない方が良いと思います。
...
Dは尺度定数です。D=1.702や1.7がよく用いられます。これはIRTモデルがもともと正規累積モデル (Nromal Ogive Model, NOM) として提案されていたため，その曲線に近似させるための定数です。正直なくても推定には影響しないのですが，識別力の値が変わってしまい，Dが異なるデータとは数値を直接比較できなくなります。もちろんD=１で推定したパラメタに1.702をかければ，D＝1.702で推定した結果と概ね一致するはずです。
